...
use cmera\addons\middleware\Addons;
use cmera\addons\Service;
use cmera\helper\FileHelper;
use think\Exception;
use think\facade\Db;
...
\think\Console::starting(function (\think\Console $console) {
    $console->addCommands([
        'addons:config' => '\\cmera\\addons\\command\\SendConfig',
        'auth:config' => '\\cmera\\auth\\command\\SendConfig',
        'curd:config' => '\\cmera\\auth\\command\\SendConfig'
    ]);
});
...
if (!function_exists('get_addons_autoload_config')) {
    ...
    function get_addons_autoload_config($chunk = false)
    {
        ...
        $route = [];
        // 读取插件目录及钩子列表
        $base = get_class_methods("\\cmera\\Addons");
